...
# calculando
medias <- sapply(resultados_simulacao, function(resultados) {
  list(beta0_hat = mean(resultados$beta0_hat), beta1_hat =mean(resultados$beta1_hat))
})

variancias <- sapply(resultados_simulacao, function(resultados) {
  list(beta0_hat = var(resultados$beta0_hat), beta1_hat = var(resultados$beta1_hat))
})

correlacoes <- sapply(resultados_simulacao, function(resultados) {
  cor(resultados$beta0_hat, resultados$beta1_hat)
})
correlacoes
...
# mudou a velocidade e em quanto esta mudando
# ir mudando a districao do erro para ver o que vai acontecendo
# verifocar se os estimadores convergem em probabilidade para os parametros (beta0, beta1)